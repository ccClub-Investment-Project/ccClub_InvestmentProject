    ...
    try:
        ...
        line_bot_api.reply_message(
            ReplyMessageRequest(
                ...
                messages=[error_message]  # Ensure messages is passed as a list
            )
        )
        user_states[user_id] = None
    ...
    try:
        ...
        if keywords:
            ...
            line_bot_api.reply_message(
                ReplyMessageRequest(
                    reply_token=event.reply_token,
                    messages=[TextMessage(text=message)]  # Ensure messages is passed as a list
                )
            )
        else:
            prompt_message = TextMessage(text="請輸入有效的關鍵字，用逗號分隔:")
            line_bot_api.reply_message(
                ReplyMessageRequest(
                    reply_token=event.reply_token,
                    messages=[prompt_message]  # Ensure messages is passed as a list
                )
            )
    except Exception as e:
        logging.error(f"Error in handle_keywords_input: {e}")
